...
void KopeteRunner::match(Plasma::RunnerContext& context)
{
	...
	i.next();
        ...
        // We don't want skip unreachable contacts
...
void KopeteRunner::loadData()
{
    ...
    QDBusReply<QStringList> replyContacts = QDBusConnection::sessionBus().call(request);
    // If the request succeeded
    if (replyContacts.isValid())
    {
        // Mark that we are loading
        m_loaded = true;
        // Get the contact list
        const QStringList& contacts = replyContacts.value();
